# =============================================================================
# sensitivity_w_b3.py  —  Hybrid Belief Update T-Sensitivity (Benchmark 3)
# Project  : ELS_NEW / P_Base3  (Gorelick Pump-and-Treat Remediation)
# Plan ref : how_improve_performance.md §2.7 Item 5; experiment_plan.md §6.3
# Ported   : from P_Base2/scr/sensitivity_w_b2.py — imports and paths adapted for B3
# Date     : 2026-04-24
# Usage    : python scr/sensitivity_w_b3.py
# Outputs  : output/sensitivity_w_results_b3.csv
#            output/sensitivity_w_report_b3.md
# =============================================================================
"""
T-Sensitivity Analysis for Benchmark 3 (Gorelick Pump-and-Treat):
ELS_Phil_T and ELS_Int_T with w ∈ {0.0, 0.25, 0.5, 0.75, 1.0}.

w=0.0 → pure static ELS update (T_B3_STAT not used)
w=0.5 → recommended hybrid blend (Priority 2 default)
w=1.0 → full Markovian Bayes update using T_B3_STAT

Key B3 differences from B2:
  T=30, CLIMATE_SWITCH_STEP=15, N_OBS=20, N=5, M=3
  Conditions: stable dynamics (stationary) / faster plume spreading at t=15 (nonstationary)
  U_T ≈ 0.5–0.7 (plume transport model uncertainty — heterogeneous subsurface)

Seeding: M_IDX_BASE=100 to avoid collision with run_benchmark_b3 (m_idx 0-15).
"""
...
from run_benchmark_b3 import _run_episode, _update_belief
...
def main():
    ...
    print("=" * 72)
    print("sensitivity_w_b3.py — T-Sensitivity Analysis (Benchmark 3 P&T)")
    ...
    print(f"\n{'='*72}")
    print(f"B3 sensitivity run complete in {elapsed:.1f}s  ({elapsed/60:.1f} min)")

    # Save CSV
    csv_path = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b3.csv")
    ...
    md_path = os.path.join(OUTPUT_DIR, "sensitivity_w_report_b3.md")
