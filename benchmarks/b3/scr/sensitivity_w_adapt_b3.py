# =============================================================================
# sensitivity_w_adapt_b3.py  —  Adaptive w Trade-off Curve (Benchmark 3 — P&T)
# Project  : ELS_NEW / P_Base3  (Gorelick Pump-and-Treat Remediation)
# Plan ref : BDT_concept_impr.md §3.1; P_Base/docs/summary_w_update_B2.md
# Ported   : from P_Base2/scr/sensitivity_w_adapt_b2.py — B3 imports and paths
# Date     : 2026-04-24
# Usage    : python scr/sensitivity_w_adapt_b3.py
# Outputs  : output/sensitivity_w_adapt_results_b3.csv
#            output/sensitivity_w_adapt_report_b3.md
#            output/sensitivity_w_report_b3.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b3.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 3 (Gorelick Pump-and-Treat).

Compares three method families:
  1. Fixed-w sweep (ELS_Phil_T, ELS_Int_T)  — from sensitivity_w_results_b3.csv
  2. Adaptive-w (ELS_Phil_Adapt, ELS_Int_Adapt) — eta ∈ {0.1, 0.5, 1.0}
  3. Reference points (ELS_Phil, SARSOP, AlwaysRestricted) — from run_benchmark_b3.py

B3 context: Under higher T-uncertainty (U_T ≈ 0.5–0.7 vs B2's 0.4–0.6),
plume transport models are structurally uncertain due to heterogeneous
subsurface, kriging uncertainty and sorption parameter errors.  The adaptive
mechanism is predicted to converge to w_mean < B2 (predicted ~0.70–0.80),
demonstrating that plume model inaccuracy pushes the agent toward
robust (lower w) belief updating.

The w trajectory post-switch (steps 15–29) should show a more pronounced
w decrease in B3 than B2 as T_B3_SHIFT dynamics diverge more strongly
from the T_B3_STAT predictions.

Seeding: M_IDX_BASE=200 to avoid collision with run_benchmark_b3 (0–15)
         and sensitivity_w_b3 (100–109).
"""
...
_BASE3 = os.path.dirname(_HERE)
if _HERE not in sys.path:
    sys.path.insert(0, _HERE)

from pomdp_env_b3 import (
...
from els_adapt_b3 import ELSPhilAdapt, ELSIntAdapt
...
W_INIT     = 0.5
M_IDX_BASE = 200
OUTPUT_DIR = os.path.join(_BASE3, "output")
IMAGE_DIR  = os.path.join(_BASE3, "image")
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(IMAGE_DIR,  exist_ok=True)

# Reference values — to be updated after running run_benchmark_b3.py
# Placeholder values will be replaced once benchmark run completes.
# These are used only for figure reference lines.
...
# If raw_results_b3.npz is present, extract reference values from it
def _try_load_refs() -> None:
    npz_path = os.path.join(OUTPUT_DIR, "raw_results_b3.npz")
    ...
    try:
        ...
        for method in ["SARSOP", "AlwaysRestricted", "ELS_Phil", "ELS_Int"]:
            for cond in ["stationary", "nonstationary"]:
                ...
                if key in data:
                    rews = data[key]            # (N_MC, T)
                    ucum = float((rews * discount).sum(axis=1).mean())
...
# ---------------------------------------------------------------------------
# Episode runner — passes action to phys_belief_update
# ---------------------------------------------------------------------------
...
def _run_adapt_mc(
    ...
    return {
        ...
        "w_mean":      w_mean_ep,
        "w_final":     w_final,
        "w_traj_mean": w_traj_mean,
    }


# ---------------------------------------------------------------------------
# Load fixed-w reference CSV
# ---------------------------------------------------------------------------
...
def _make_figure(
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 3 — Gorelick P&T, N_MC=200)",
    ...
    for col, cond in enumerate(["stationary", "nonstationary"]):
        ...
        for method, color, mk in [
            ...
            for i, r in enumerate(sub_sorted):
                ...
                ax_top.plot(r["w_mean"], r["ucum_mean"],
                            marker=mk, markersize=ms, color=color,
        ...
        # Reference lines (only plot if value is available)
        ...
        cond_label = "stable dynamics" if cond == "stationary" else "faster spreading at t=15"
        ...
        ax_top.set_ylabel("U_cum (mean discounted $k/year)")
        ax_top.set_xlim(-0.05, 1.05)
        ax_top.legend(fontsize=7, loc="lower right", ncol=2)
        ax_top.grid(True, alpha=0.3)

        # w trajectory
        for method, color in [("ELS_Phil_Adapt", C_PHIL_ADAPT), ("ELS_Int_Adapt", C_INT_ADAPT)]:
            sub = [
                r for r in adapt_results
                if r["name"] == method and r["stationary"] == (cond == "stationary")
            ]
        ...
        if cond == "nonstationary":
            ax_bot.axvline(CLIMATE_SWITCH_STEP, color="red", ls="--", lw=1.5,
                           alpha=0.8, label="faster spreading onset")
...
def _save_report(
    ...
    sarsop_s  = REFS["SARSOP"]["stationary"]
    sarsop_ns = REFS["SARSOP"]["nonstationary"]
    always_s  = REFS["AlwaysRestricted"]["stationary"]
    always_ns = REFS["AlwaysRestricted"]["nonstationary"]
    phil_s    = REFS["ELS_Phil"]["stationary"]
    phil_ns   = REFS["ELS_Phil"]["nonstationary"]
    ...
    lines += [
        ...
        "|---|---|---|",
        f"| SARSOP | {_fmt(sarsop_s)} | {_fmt(sarsop_ns)} |",
        f"| AlwaysRestricted | {_fmt(always_s)} | {_fmt(always_ns)} |",
        f"| ELS_Phil (w=0) | {_fmt(phil_s)} | {_fmt(phil_ns)} |",
    ]
    with open(out_path, "w") as f:
        f.write("\n".join(lines) + "\n")
...
def main(n_mc: int = N_MC) -> None:
    ...
    print("=" * 72)
    print("sensitivity_w_adapt_b3.py — Adaptive w Trade-off (Benchmark 3 P&T)")
    print(f"  N_MC={n_mc}  T={T}  γ={GAMMA}  SEED={SEED}")
    print(f"  eta values : {ETA_VALUES}  w_init={W_INIT}")
    print("=" * 72)

    # ------------------------------------------------------------------
    # 0. Load reference values from benchmark run if available
    # ------------------------------------------------------------------
    _try_load_refs()

    # ------------------------------------------------------------------
    # 1. Load or generate fixed-w reference
    # ------------------------------------------------------------------
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b3.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] sensitivity_w_results_b3.csv not found — running sensitivity_w_b3.py ...")
        import sensitivity_w_b3
        sensitivity_w_b3.main()
    else:
    ...
    print(f"\n  [B3 adaptive runs complete: {elapsed:.1f}s]")

    # ------------------------------------------------------------------
    # 3. Save outputs
    # ------------------------------------------------------------------
    print("\n[3] Saving outputs ...")
    csv_path    = os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b3.csv")
    report_path = os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b3.md")
    fig_path    = os.path.join(IMAGE_DIR,  "fig_w_adapt_tradeoff_b3.png")
    ...
    print(f"\n{'='*72}")
    print(f"sensitivity_w_adapt_b3.py — complete  ({time.time() - t_start:.1f}s)")
    print(f"{'='*72}")
