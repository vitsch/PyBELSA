# =============================================================================
# figures.py  —  Publication-Quality Figures for Benchmark 1
# Project  : ELS_NEW / P_Base  (Benchmark 5 — NCP GRACE Gorelick)
# Plan ref : experiment_plan.md §5 Step 6
# Date     : 2026-04-25
# Usage    : python scr/figures_b2.py  (in P_Base2/scr/)
# Inputs   : output/raw_results.npz
# Outputs  : image/fig1_b5_ucum_comparison.png
#            image/fig2_b5_pfail.png
#            image/fig3_b5_nonstationarity.png
#            image/fig4_b5_climate_scenarios.png
#            image/convergence_check_b5.png
...
from pomdp_env_b5 import T, GAMMA, SEED, FAILURE_STATE, CLIMATE_SWITCH_STEP
...
# Paths
# ---------------------------------------------------------------------------
NPZ_PATH   = os.path.join(os.path.dirname(_HERE), "output", "raw_results_b5.npz")
...
METHOD_COLORS = {
    ...
    "SDP_S_Misspec":   "#3B7A57",   # hunter green (misspec ablation)
}
...
COND_LABEL  = {"stationary": "long-term average dynamics (base)", "nonstationary": "SSP5-8.5 demand increase (shifted)"}
...
def fig1_ucum(res: dict) -> str:
    ...
    return _savefig(fig, "fig1_b5_ucum_comparison.png")
...
def fig2_pfail(res: dict) -> str:
    ...
    fig.suptitle("Fig 2 — Storage Failure Metrics (B2 MAR) by Policy and Condition",
                 fontsize=TITLE_FS + 1)
    return _savefig(fig, "fig2_b5_pfail.png")
...
def fig3_nonstationarity(npz) -> str:
    ...
    nonstationary condition.  Vertical dashed line at t = CLIMATE_SWITCH_STEP.
    ...
    for ax, cond in zip(axes, CONDITIONS):
        ...
        ax.axvline(CLIMATE_SWITCH_STEP, color="firebrick", linestyle="--",
                   linewidth=1.2, alpha=0.75, label=f"SSP585 onset (t={CLIMATE_SWITCH_STEP})",
                   zorder=4)
    ...
    fig.suptitle("Fig 3 — Per-Step Mean Reward Trajectory",
                 fontsize=TITLE_FS + 1)
    return _savefig(fig, "fig3_b5_nonstationarity.png")
...
def fig4_evoi(res: dict) -> str:
    ...
    fig.suptitle("Fig 4 — Climate Scenario Value (B2 MAR) (EVOI) Decomposition",
                 fontsize=TITLE_FS + 1)
    return _savefig(fig, "fig4_b5_climate_scenarios.png")
...
def fig5_convergence(res: dict) -> str:
    ...
    fig.suptitle("Fig 5 — Convergence Check: SE of Mean U_cum vs N_MC",
                 fontsize=TITLE_FS + 1)
    return _savefig(fig, "convergence_check_b5.png")
