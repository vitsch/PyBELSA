# =============================================================================
# sensitivity_w_adapt_b5.py  —  Adaptive w Trade-off Curve (Benchmark 5 — NCP)
...
# Date     : 2026-04-25
# Usage    : python scr/sensitivity_w_adapt_b5.py
# Outputs  : output/sensitivity_w_adapt_results_b5.csv
#            output/sensitivity_w_adapt_report_b5.md
#            output/sensitivity_w_report_b5.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b5.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 5 (North China Plain GRACE).
...
Seeding: M_IDX_BASE=200 to avoid collision with run_benchmark_b5 (0–15)
         and sensitivity_w_b5 (100–109).
"""
...
_BASE5 = os.path.dirname(_HERE)
if _HERE not in sys.path:
    sys.path.insert(0, _HERE)

from pomdp_env_b5 import (
...
from els_adapt_b5 import ELSPhilAdapt, ELSIntAdapt
...
W_INIT     = 0.5
M_IDX_BASE = 200
OUTPUT_DIR = os.path.join(_BASE5, "output")
IMAGE_DIR  = os.path.join(_BASE5, "image")
...
def _try_load_refs() -> None:
    npz_path = os.path.join(OUTPUT_DIR, "raw_results_b5.npz")
...
def _make_figure(fixed_records, adapt_results, out_path):
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 5 — NCP GRACE, N_MC=200)",
    ...
    for col, cond in enumerate(["stationary", "nonstationary"]):
        ...
        cond_label = "long-term avg dynamics" if cond == "stationary" \
                     else "SSP5-8.5 demand increase at t=15"
...
def main(n_mc=N_MC):
    t_start = time.time()
    print("=" * 72)
    print("sensitivity_w_adapt_b5.py — Adaptive w Trade-off (Benchmark 5 NCP GRACE)")
    ...
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b5.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] CSV not found — running sensitivity_w_b5.py ...")
        import sensitivity_w_b5
        sensitivity_w_b5.main()
    else:
    ...
    print(f"\n  [B5 adaptive runs complete: {elapsed:.1f}s]")

    print("\n[3] Saving outputs ...")
    _save_csv(adapt_results, os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b5.csv"))
    _save_report(fixed_records, adapt_results,
                 os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b5.md"))

    print("\n[4] Generating trade-off figure ...")
    try:
        _make_figure(fixed_records, adapt_results,
                     os.path.join(IMAGE_DIR, "fig_w_adapt_tradeoff_b5.png"))
    except Exception as exc:
        print(f"  WARNING: figure generation failed: {exc}")

    print(f"\n{'='*72}")
    print(f"sensitivity_w_adapt_b5.py — complete  ({time.time() - t_start:.1f}s)")
    print(f"{'='*72}")
