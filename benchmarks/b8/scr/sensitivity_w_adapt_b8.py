...
# Project  : ELS_NEW / P_Base6  (Stampriet Transboundary Aquifer STAS-Based)
...
# Outputs  : output/sensitivity_w_adapt_results_b8.csv
#            output/sensitivity_w_adapt_report_b8.md
#            output/sensitivity_w_report_b6.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b8.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 8 (Stampriet Transboundary Aquifer STAS).
...
from els_adapt_b8 import ELSPhilAdapt, ELSIntAdapt
...
def _try_load_refs() -> None:
    npz_path = os.path.join(OUTPUT_DIR, "raw_results_b8.npz")
...
def _make_figure(fixed_records, adapt_results, out_path):
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 6 — Stampriet Transboundary Aquifer STAS, N_MC=200)",
...
def main(n_mc=N_MC):
    t_start = time.time()
    print("=" * 72)
    print("sensitivity_w_adapt_b7.py — Adaptive w Trade-off (Benchmark 8 Stampriet Transboundary Aquifer STAS)")
    ...
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b8.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] CSV not found — running sensitivity_w_b8.py ...")
        import sensitivity_w_b8
        sensitivity_w_b8.main()
    else:
    ...
    print(f"\n  [B8 adaptive runs complete: {elapsed:.1f}s]")

    print("\n[3] Saving outputs ...")
    _save_csv(adapt_results, os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b8.csv"))
    _save_report(fixed_records, adapt_results,
                 os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b8.md"))

    print("\n[4] Generating trade-off figure ...")
    try:
        _make_figure(fixed_records, adapt_results,
                     os.path.join(IMAGE_DIR, "fig_w_adapt_tradeoff_b8.png"))
