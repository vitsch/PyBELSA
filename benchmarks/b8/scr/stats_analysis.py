...
from pomdp_env_b8 import T, GAMMA, SEED, FAILURE_STATE, N_MC
...
def write_tables(res: dict) -> None:
    ...
    with open(os.path.join(TABLES_DIR, "table_ucum_b8.csv"), "w") as f:
        f.write("\n".join(rows_ucum) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_pfail_b8.csv"), "w") as f:
        f.write("\n".join(rows_pf) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_regret_b8.csv"), "w") as f:
        f.write("\n".join(rows_reg) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_pairwise_b8.csv"), "w") as f:
        f.write("\n".join(rows_pw) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_nonstationarity_b8.csv"), "w") as f:
        f.write("\n".join(rows_ns) + "\n")
...
def main() -> None:
    ...
    report_path = os.path.join(OUTPUT_DIR, "stat_report_b8.md")
