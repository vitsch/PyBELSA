# =============================================================================
# els_adapt_b8.py  —  ELS Adaptive Model-Trust Variants (Benchmark 7 — Stampriet)
# Project  : ELS_NEW / P_Base8  (Stampriet Aquifer: Saltwater Intrusion Tipping Point)
# Plan ref : BDT_concept_impr.md §3.1; P_Base/docs/experiment_plan2.md §12.2
# Ported   : from P_Base6/scr/els_adapt_b6.py — only imports changed for B8
# Date     : 2026-04-25
# Usage    : from els_adapt_b8 import ELSPhilAdapt, ELSIntAdapt
# =============================================================================
"""
ELS_Phil_Adapt and ELS_Int_Adapt for Benchmark 7 (Stampriet STAS Multi-Agent).

Adaptive model-trust weight w: after each observation, w is updated in
logit space based on log-likelihood ratio (T_B8_STAT-propagated vs static):

    ll_T  = log P(obs | T_stat[a]ᵀ · b)    [T_B8_STAT prediction]
    ll_s  = log P(obs | b)                   [static prior]
    δ     = η · (ll_T − ll_s)
    logit(w_new) = logit(w) + δ
    w_new  = clip(sigmoid(logit(w_new)), w_min, w_max)

Key B8 differences vs B6:
  - Domain: Stampriet aquifer with multi-agent game-theoretic T uncertainty
  - T = 30 steps (CLIMATE_SWITCH_STEP = 15 — governance breakdown / defection onset)
  - N_OBS = 20 bins (GRACE TWSA likelihood; OBS_BINS in mm WE TWSA space)
  - T_stat = T_B8_STAT (pre-climate coastal dynamics with tipping structure)
  - T_shift = T_B8_SHIFT (amplified tipping under sea-level rise)
  ...
  - L_PHIL = avg(σ=35 GRACE mascon, σ=55 bore network)
  - SIGMA_OBS = 0.50 (log-space; log-normal observation model — B8 novelty)

Expected adaptive behaviour at B8:
  ...
  - η=0.1: w_mean ≈ 0.60–0.70 (detects governance-shift signal but conservative)
  - η=0.5: w_mean ≈ 0.70–0.80 (effective T-signal at governance breakdown)
  - P_fail improvement expected: adaptive w correctly increases T-trust before
    entering the defection detection, leading to earlier cooperative/emergency switching
  - This is the predicted B8 "success story" for adaptive w

Inheritance chain (B8):
  ELSPhilAdapt  ←  ELSPhilT  ←  ELSPhil   (all from P_Base8/scr/els_methods.py)
  ELSIntAdapt   ←  ELSIntT   ←  ELSInt

All parent classes import from pomdp_env_b8 — no additional changes needed.
"""
...
from pomdp_env_b8 import T_stat, L_PHIL, L_PRES


# ===========================================================================
# 1.  ELSPhilAdapt  —  Philosophical + Online-Adaptive w  (B8)
# ===========================================================================

class ELSPhilAdapt(ELSPhilT):
    """
    Philosophical framework (B8 Stampriet) with adaptive model-trust weight.

    Uses T_B8_STAT as the reference model. L_PHIL is model-averaged likelihood
...
# ===========================================================================
# 2.  ELSIntAdapt  —  Integrated Bernoulli + Online-Adaptive w  (B8)
# ===========================================================================

class ELSIntAdapt(ELSIntT):
    """
    Integrated Bernoulli framework (B8 Stampriet) with adaptive model-trust weight.
...
def validate(verbose: bool = True) -> None:
    ...
    from pomdp_env_b8 import N, M, T as T_B8, SEED as _SEED
    ...
    for name, policy, ep_seed in tests:
        ...
        assert len(policy.w_trajectory) == T_B8
    ...
    if verbose:
        print("ELS-Adapt B8 smoke tests passed.")
