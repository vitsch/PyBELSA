# =============================================================================
# sensitivity_w_adapt_b6.py  —  Adaptive w Trade-off Curve (Benchmark 6 — NCP)
...
# Date     : 2026-04-25
# Usage    : python scr/sensitivity_w_adapt_b6.py
# Outputs  : output/sensitivity_w_adapt_results_b6.csv
#            output/sensitivity_w_adapt_report_b6.md
#            output/sensitivity_w_report_b6.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b6.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 6 (Karst Aquifer).
...
from els_adapt_b6 import ELSPhilAdapt, ELSIntAdapt
...
IMAGE_DIR  = os.path.join(_BASE6, "image")
...
def _try_load_refs() -> None:
    npz_path = os.path.join(OUTPUT_DIR, "raw_results_b6.npz")
...
def _make_figure(fixed_records, adapt_results, out_path):
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 6 — Karst Aquifer Dual-Porosity, N_MC=200)",
    ...
    for col, cond in enumerate(["stationary", "nonstationary"]):
        ...
        cond_label = "long-term avg karst dynamics" if cond == "stationary" \
                     else "conduit desiccation at t=15"
...
def main(n_mc=N_MC):
    t_start = time.time()
    print("=" * 72)
    print("sensitivity_w_adapt_b6.py — Adaptive w Trade-off (Benchmark 6 Karst Aquifer Dual-Porosity)")
    ...
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b6.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] CSV not found — running sensitivity_w_b6.py ...")
        import sensitivity_w_b6
        sensitivity_w_b6.main()
    else:
    ...
    print("\n[3] Saving outputs ...")
    _save_csv(adapt_results, os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b6.csv"))
    _save_report(fixed_records, adapt_results,
                 os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b6.md"))

    print("\n[4] Generating trade-off figure ...")
    try:
        _make_figure(fixed_records, adapt_results,
                     os.path.join(IMAGE_DIR, "fig_w_adapt_tradeoff_b6.png"))
    except Exception as exc:
        print(f"  WARNING: figure generation failed: {exc}")

    print(f"\n{'='*72}")
    print(f"sensitivity_w_adapt_b6.py — complete  ({time.time() - t_start:.1f}s)")
    print(f"{'='*72}")
