# =============================================================================
# sensitivity_w_adapt_b7.py  —  Adaptive w Trade-off Curve (Benchmark 9 — NCP)
# Project  : ELS_NEW / P_Base6  (Indus Basin / HKH Glacial Meltwater Recharge-Based)
...
# Outputs  : output/sensitivity_w_adapt_results_b9.csv
#            output/sensitivity_w_adapt_report_b9.md
#            output/sensitivity_w_report_b6.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b9.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 9 (Indus Basin / HKH Glacial Meltwater Recharge).
...
W_INIT     = 0.5
M_IDX_BASE = 200
OUTPUT_DIR = os.path.join(_BASE9, "output")
IMAGE_DIR  = os.path.join(_BASE9, "image")
...
def _try_load_refs() -> None:
    npz_path = os.path.join(OUTPUT_DIR, "raw_results_b9.npz")
...
def _make_figure(fixed_records, adapt_results, out_path):
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 9 — Indus Basin / HKH Glacial Meltwater Recharge, N_MC=200)",
    ...
    for col, cond in enumerate(["stationary", "nonstationary"]):
        ...
        cond_label = "current HKH recharge era (T_stat)" if cond == "stationary" \
                     else "continuous glacial retreat: T(t)=(1-α)T_stat+αT_shift"
        ...
        if cond == "nonstationary":
            # B9: continuous drift — shade to indicate accelerating T divergence
            ax_bot.axhspan(0.0, 1.05, alpha=0.04, color="red",
                           label="continuous glacial retreat →")
...
def main(n_mc=N_MC):
    t_start = time.time()
    print("=" * 72)
    print("sensitivity_w_adapt_b7.py — Adaptive w Trade-off (Benchmark 9 Indus Basin / HKH Glacial Meltwater Recharge)")
    ...
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b9.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] CSV not found — running sensitivity_w_b9.py ...")
        import sensitivity_w_b9
        sensitivity_w_b9.main()
    else:
    ...
    print(f"\n  [B9 adaptive runs complete: {elapsed:.1f}s]")

    print("\n[3] Saving outputs ...")
    _save_csv(adapt_results, os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b9.csv"))
    _save_report(fixed_records, adapt_results,
                 os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b9.md"))

    print("\n[4] Generating trade-off figure ...")
    try:
        _make_figure(fixed_records, adapt_results,
                     os.path.join(IMAGE_DIR, "fig_w_adapt_tradeoff_b9.png"))
