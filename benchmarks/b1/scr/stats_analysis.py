...
# Date     : 2026-04-22
# Usage    : python scr/stats_analysis.py
...
from pomdp_env import T, GAMMA, SEED, FAILURE_STATE, N_MC
...
NPZ_PATH   = os.path.join(os.path.dirname(_HERE), "output", "raw_results.npz")
...
def compute_episode_metrics(npz: np.lib.npyio.NpzFile, name: str, cond: str,
    ...
    ucum_A = (rewards[:, :30] * _DISC[:30]).sum(axis=1)             # regime A
    ucum_B = (rewards[:, 30:] * _DISC[30:]).sum(axis=1)             # regime B
...
def write_tables(res: dict) -> None:
    ...
    with open(os.path.join(TABLES_DIR, "table_ucum.csv"), "w") as f:
        f.write("\n".join(rows_ucum) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_pfail.csv"), "w") as f:
        f.write("\n".join(rows_pf) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_regret.csv"), "w") as f:
        f.write("\n".join(rows_reg) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_pairwise.csv"), "w") as f:
        f.write("\n".join(rows_pw) + "\n")
    ...
    with open(os.path.join(TABLES_DIR, "table_nonstationarity.csv"), "w") as f:
        f.write("\n".join(rows_ns) + "\n")
...
def main() -> None:
    ...
    report_path = os.path.join(OUTPUT_DIR, "stat_report_benchmark.md")
