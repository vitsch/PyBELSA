...
# Project  : ELS_NEW / P_Base6  (Sahel / Lake Chad Basin Quaternary Aquifer-Based)
...
# Outputs  : output/sensitivity_w_adapt_results_b10.csv
#            output/sensitivity_w_adapt_report_b10.md
#            output/sensitivity_w_report_b6.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b10.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 10 (Sahel / Lake Chad Basin Quaternary Aquifer).
...
_BASE8 = os.path.dirname(_HERE)
if _HERE not in sys.path:
    sys.path.insert(0, _HERE)

from pomdp_env_b10 import (
...
from els_adapt_b10 import ELSPhilAdapt, ELSIntAdapt
...
W_INIT     = 0.5
M_IDX_BASE = 200
OUTPUT_DIR = os.path.join(_BASE8, "output")
IMAGE_DIR  = os.path.join(_BASE8, "image")
...
def _try_load_refs() -> None:
    npz_path = os.path.join(OUTPUT_DIR, "raw_results_b10.npz")
...
def _make_figure(fixed_records, adapt_results, out_path):
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 6 — Sahel / Lake Chad Basin Quaternary Aquifer, N_MC=200)",
    ...
    for col, cond in enumerate(["stationary", "nonstationary"]):
        ...
        cond_label = "Nash-equilibrium STAS dynamics" if cond == "stationary" \
                     else "governance breakdown (defection) at t=15"
...
def main(n_mc=N_MC):
    t_start = time.time()
    print("=" * 72)
    print("sensitivity_w_adapt_b7.py — Adaptive w Trade-off (Benchmark 10 Sahel / Lake Chad Basin Quaternary Aquifer)")
    ...
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b10.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] CSV not found — running sensitivity_w_b10.py ...")
        import sensitivity_w_b10
        sensitivity_w_b10.main()
    else:
    ...
    print(f"\n  [B10 adaptive runs complete: {elapsed:.1f}s]")

    print("\n[3] Saving outputs ...")
    _save_csv(adapt_results, os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b10.csv"))
    _save_report(fixed_records, adapt_results,
                 os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b10.md"))

    print("\n[4] Generating trade-off figure ...")
    try:
        _make_figure(fixed_records, adapt_results,
                     os.path.join(IMAGE_DIR, "fig_w_adapt_tradeoff_b10.png"))
