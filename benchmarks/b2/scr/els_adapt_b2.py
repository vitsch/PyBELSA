# =============================================================================
# els_adapt_b2.py  —  ELS Adaptive Model-Trust Variants (Benchmark 2 — MAR)
# Project  : ELS_NEW / P_Base2  (Arizona/Spain Managed Aquifer Recharge)
# Plan ref : BDT_concept_impr.md §3.1; P_Base/docs/summary_w_update.md
# Ported   : from P_Base/scr/els_adapt.py — only imports changed for B2
# Date     : 2026-04-24
# Usage    : from els_adapt_b2 import ELSPhilAdapt, ELSIntAdapt
# =============================================================================
"""
ELS_Phil_Adapt and ELS_Int_Adapt for Benchmark 2 (MAR Arizona/Spain).

Adaptive model-trust weight w: after each observation, w is updated in
logit space based on log-likelihood ratio (T_SSP245-propagated vs static):

    ll_T  = log P(obs | T_SSP245[a]ᵀ · b)    [SSP2-4.5 T-prediction]
    ll_s  = log P(obs | b)                     [static prior]
    δ     = η · (ll_T − ll_s)
    logit(w_new) = logit(w) + δ
    w_new  = clip(sigmoid(logit(w_new)), w_min, w_max)

Key B2 differences vs B1:
  - T = 30 steps (CLIMATE_SWITCH_STEP = 15)
  - N_OBS = 20 bins (L_PHIL and L_PRES are 5×20)
  - T_stat = T_SSP245 (bidirectional — recovery possible)
  - Observation noise σ = 35 GL-equiv (wider than B1)
  - U_MIN = -100.0 in els_methods.py (R includes negative values)

Inheritance chain (B2):
  ELSPhilAdapt  ←  ELSPhilT  ←  ELSPhil   (all from P_Base2/scr/els_methods.py)
  ELSIntAdapt   ←  ELSIntT   ←  ELSInt

All parent classes already import from pomdp_env_b2 — no additional changes needed.
"""
...
# Import B2 variants of ELSPhilT / ELSIntT (which already use pomdp_env_b2 internals)
from els_methods import (
    ELSPhilT, ELSIntT, ELSInt,
    W_TRANS, SEED, C_TEF,
)
from pomdp_env_b2 import T_stat, L_PHIL, L_PRES


# ===========================================================================
# 1.  ELSPhilAdapt  —  Philosophical + Online-Adaptive w  (B2)
# ===========================================================================

class ELSPhilAdapt(ELSPhilT):
    """
    Philosophical framework (B2 MAR) with adaptive model-trust weight.

    Uses T_SSP245 (= T_stat) as the reference transition model for
    prediction accuracy measurement. L_PHIL is the model-averaged
    likelihood (σ=30.0 precision bore + σ=45.0 diffuse report).
    ...
    def phys_belief_update(
        ...
        # Step 1: belief update with current w (ELSPhilT → L_PHIL + T_stat from B2)
        b_new = super().phys_belief_update(b, obs, action)

        # Step 2-3: adapt w using L_PHIL and T_SSP245
...
# ===========================================================================
# 2.  ELSIntAdapt  —  Integrated Bernoulli + Online-Adaptive w  (B2)
# ===========================================================================

class ELSIntAdapt(ELSIntT):
    """
    Integrated Bernoulli framework (B2 MAR) with adaptive model-trust weight.

    P_hyb = (1−α_prev)·L_PHIL + α_prev·L_PRES is used for both belief
    update and T-accuracy measurement (self-consistent with B2 Bernoulli mixing).
    ...
    def phys_belief_update(
        ...
        # Step 1: belief update with current w (ELSIntT uses P_hyb + T_stat from B2)
        b_new = super().phys_belief_update(b, obs, action)
...
def validate(verbose: bool = True) -> None:
    """Smoke-test each ELS-Adapt policy through one B2 episode."""
    import pomdp_env_b2 as env_mod
    from pomdp_env_b2 import N, M, T as T_B2, SEED as _SEED
    ...
    for name, policy, ep_seed in tests:
        ...
        assert len(policy.w_trajectory) == T_B2
    ...
    if verbose:
        print("ELS-Adapt B2 smoke tests passed.")
