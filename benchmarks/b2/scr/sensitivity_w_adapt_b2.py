# =============================================================================
# sensitivity_w_adapt_b2.py  —  Adaptive w Trade-off Curve (Benchmark 2 — MAR)
# Project  : ELS_NEW / P_Base2  (Arizona/Spain Managed Aquifer Recharge)
# Plan ref : BDT_concept_impr.md §3.1; P_Base/docs/summary_w_update.md §7
# Ported   : from P_Base/scr/sensitivity_w_adapt.py — B2 imports and paths
# Date     : 2026-04-24
# Usage    : python scr/sensitivity_w_adapt_b2.py
# Outputs  : output/sensitivity_w_adapt_results_b2.csv
#            output/sensitivity_w_adapt_report_b2.md
#            output/sensitivity_w_report_b2.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b2.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 2 (MAR Arizona/Spain).

Compares three method families:
  1. Fixed-w sweep (ELS_Phil_T, ELS_Int_T)  — from sensitivity_w_results_b2.csv
  2. Adaptive-w (ELS_Phil_Adapt, ELS_Int_Adapt) — eta ∈ {0.1, 0.5, 1.0}
  3. Reference points (ELS_Phil, SARSOP, AlwaysRestricted) — from raw_results_b2.npz

B2 context: Under medium T-uncertainty (U_T ≈ 0.4–0.6), the adaptive mechanism
is predicted to find w < 0.9 (unlike B1 where w → 1.0), demonstrating that the
SSP5-8.5 climate shift reduces T_SSP245 prediction accuracy and pushes w down.

The w trajectory post-switch (steps 15–29) should show adaptation toward lower w
when SSP5-8.5 dynamics diverge from the T_SSP245 predictions.

Seeding: M_IDX_BASE=200 to avoid collision with run_benchmark_b2 (0–15)
         and sensitivity_w_b2 (100–109).
"""
...
from els_adapt_b2 import ELSPhilAdapt, ELSIntAdapt
...
IMAGE_DIR  = os.path.join(_BASE2, "image")
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(IMAGE_DIR,  exist_ok=True)

# Reference values from run_benchmark_b2.py (stat_report_b2.md)
REFS = {
    "SARSOP":           {"stationary": 1281.5, "nonstationary": 1181.6},
    "AlwaysRestricted": {"stationary":  928.3, "nonstationary":  810.9},
    "ELS_Phil":         {"stationary":  755.9, "nonstationary":  666.9},
    "ELS_Int":          {"stationary":  698.8, "nonstationary":  648.4},
}
...
def _make_figure(
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 2 — MAR Arizona/Spain, N_MC=200)",
    ...
    for col, cond in enumerate(["stationary", "nonstationary"]):
        ...
        ax_top.set_title(f"Condition: {cond}  (SSP2-4.5{'→SSP5-8.5 at t=15' if cond=='nonstationary' else ''})",
                         fontsize=10)
        ax_top.set_xlabel("Model-trust weight  w  (or effective mean w for adaptive)")
        ax_top.set_ylabel("U_cum (mean discounted $/ML)")
...
def main(n_mc: int = N_MC) -> None:
    ...
    print("=" * 72)
    print("sensitivity_w_adapt_b2.py — Adaptive w Trade-off (Benchmark 2 MAR)")
    ...
    # ------------------------------------------------------------------
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b2.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] sensitivity_w_results_b2.csv not found — running sensitivity_w_b2.py ...")
        import sensitivity_w_b2
        sensitivity_w_b2.main()
    else:
    ...
    print(f"\n  [B2 adaptive runs complete: {elapsed:.1f}s]")

    # ------------------------------------------------------------------
    # 3. Save outputs
    # ------------------------------------------------------------------
    print("\n[3] Saving outputs ...")
    csv_path    = os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b2.csv")
    report_path = os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b2.md")
    fig_path    = os.path.join(IMAGE_DIR,  "fig_w_adapt_tradeoff_b2.png")
    ...
    print(f"\n{'='*72}")
    print(f"sensitivity_w_adapt_b2.py — complete  ({time.time() - t_start:.1f}s)")
    print(f"{'='*72}")
