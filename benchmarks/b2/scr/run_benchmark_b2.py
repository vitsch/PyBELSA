...
# Date     : 2026-04-23
# Usage    : python scr/run_benchmark_b2.py
#            python scr/run_benchmark_b2.py --dry-run   (5 episodes, quick check)
...
N_MC = 200 episodes of T = 60 steps, using GAMMA = 0.97 and SEED = 2024.
...
def _update_belief(
    ...
    if name in ("SARSOP", "PBVI"):
        ...
        T_use = T_shift if t >= CLIMATE_SWITCH_STEP else T_stat
...
def main(n_mc: int = N_MC, verbose: bool = True) -> str:
    ...
    if verbose:
        print("=" * 72)
        print("run_benchmark_b2.py — MC Evaluation (Benchmark 2)")
    ...
    out_path = os.path.join(OUTPUT_DIR, "raw_results_b2.npz")
