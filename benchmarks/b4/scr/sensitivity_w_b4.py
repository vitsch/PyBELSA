# =============================================================================
# sensitivity_w_b4.py  —  Hybrid Belief Update T-Sensitivity (Benchmark 4)
# Project  : ELS_NEW / P_Base4  (Murray-Darling Basin Groundwater SDL)
# Plan ref : how_improve_performance.md §2.7 Item 5; experiment_plan.md §6.4
# Ported   : from P_Base3/scr/sensitivity_w_b3.py — imports and paths adapted for B4
# Date     : 2026-04-24
# Usage    : python scr/sensitivity_w_b4.py
# Outputs  : output/sensitivity_w_results_b4.csv
#            output/sensitivity_w_report_b4.md
# =============================================================================
"""
T-Sensitivity Analysis for Benchmark 4 (Murray-Darling Basin SDL):
ELS_Phil_T and ELS_Int_T with w ∈ {0.0, 0.25, 0.5, 0.75, 1.0}.

w=0.0 → pure static ELS update (T_B4_STAT not used)
w=0.5 → recommended hybrid blend (Priority 2 default)
w=1.0 → full Markovian Bayes update using T_B4_STAT

Key B4 differences from B3:
  T=30, CLIMATE_SWITCH_STEP=15, N_OBS=20, N=5, M=3
  Conditions: SSP2-4.5 (stationary) / SSP5-8.5 drying at t=15 (nonstationary)
  U_T ≈ 0.6–0.8 (CMIP6 ensemble spread — higher T-uncertainty than B3)
  SIGMA_OBS=35 (wider than B3=28, CMIP6 model spread adds noise)

Seeding: M_IDX_BASE=100 to avoid collision with run_benchmark_b4 (m_idx 0-15).
"""
...
from run_benchmark_b4 import _run_episode, _update_belief
...
def main():
    ...
    print("=" * 72)
    print("sensitivity_w_b4.py — T-Sensitivity Analysis (Benchmark 4 MDB SDL)")
    ...
    print(f"\n{'='*72}")
    print(f"B4 sensitivity run complete in {elapsed:.1f}s  ({elapsed/60:.1f} min)")

    csv_path = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b4.csv")
    ...
    md_path = os.path.join(OUTPUT_DIR, "sensitivity_w_report_b4.md")
