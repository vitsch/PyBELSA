# =============================================================================
# sensitivity_w_adapt_b4.py  —  Adaptive w Trade-off Curve (Benchmark 4 — MDB SDL)
...
# Ported   : from P_Base3/scr/sensitivity_w_adapt_b3.py — B4 imports and paths
# Date     : 2026-04-24
# Usage    : python scr/sensitivity_w_adapt_b4.py
# Outputs  : output/sensitivity_w_adapt_results_b4.csv
#            output/sensitivity_w_adapt_report_b4.md
#            output/sensitivity_w_report_b4.md  (fixed-w reference, if not present)
#            image/fig_w_adapt_tradeoff_b4.png
# =============================================================================
"""
Adaptive w trade-off analysis for Benchmark 4 (Murray-Darling Basin SDL).

Compares three method families:
  1. Fixed-w sweep (ELS_Phil_T, ELS_Int_T)  — from sensitivity_w_results_b4.csv
  2. Adaptive-w (ELS_Phil_Adapt, ELS_Int_Adapt) — eta ∈ {0.1, 0.5, 1.0}
  3. Reference points (ELS_Phil, SARSOP, AlwaysRestricted) — from run_benchmark_b4.py

B4 context: Under the highest T-uncertainty so far (U_T ≈ 0.6–0.8),
CMIP6 ensemble spread (6–18% recharge variation across GCMs) makes the
SSP2-4.5 median T_stat less reliable than B3's plume transport model.
The adaptive mechanism is predicted to converge to w_mean < B3 (predicted ~0.60–0.70),
and the nonstationary w trajectory should show a more pronounced decrease
after CLIMATE_SWITCH_STEP=15 as SSP5-8.5 diverges from SSP2-4.5.

Seeding: M_IDX_BASE=200 to avoid collision with run_benchmark_b4 (0–15)
         and sensitivity_w_b4 (100–109).
"""
...
_BASE4 = os.path.dirname(_HERE)
if _HERE not in sys.path:
    sys.path.insert(0, _HERE)

from pomdp_env_b4 import (
...
from els_adapt_b4 import ELSPhilAdapt, ELSIntAdapt
...
W_INIT     = 0.5
M_IDX_BASE = 200
OUTPUT_DIR = os.path.join(_BASE4, "output")
IMAGE_DIR  = os.path.join(_BASE4, "image")
...
def _try_load_refs() -> None:
    npz_path = os.path.join(OUTPUT_DIR, "raw_results_b4.npz")
...
def _make_figure(
    ...
    fig.suptitle(
        "Adaptive vs Fixed Model-Trust Weight  (Benchmark 4 — MDB SDL, N_MC=200)",
    ...
    for col, cond in enumerate(["stationary", "nonstationary"]):
        ...
        cond_label = "SSP2-4.5 recharge" if cond == "stationary" else "SSP5-8.5 drying at t=15"
        ...
        ax_top.set_ylabel("U_cum (mean discounted GL/year-equiv)")
        ...
        if cond == "nonstationary":
            ax_bot.axvline(CLIMATE_SWITCH_STEP, color="red", ls="--", lw=1.5,
                           alpha=0.8, label="SSP5-8.5 drying onset")
...
def main(n_mc: int = N_MC) -> None:
    ...
    print("=" * 72)
    print("sensitivity_w_adapt_b4.py — Adaptive w Trade-off (Benchmark 4 MDB SDL)")
    ...
    fixed_csv = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b4.csv")
    if not os.path.exists(fixed_csv):
        print(f"\n[1] sensitivity_w_results_b4.csv not found — running sensitivity_w_b4.py ...")
        import sensitivity_w_b4
        sensitivity_w_b4.main()
    else:
    ...
    print(f"\n  [B4 adaptive runs complete: {elapsed:.1f}s]")

    print("\n[3] Saving outputs ...")
    csv_path    = os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_results_b4.csv")
    report_path = os.path.join(OUTPUT_DIR, "sensitivity_w_adapt_report_b4.md")
    fig_path    = os.path.join(IMAGE_DIR,  "fig_w_adapt_tradeoff_b4.png")
    ...
    print(f"\n{'='*72}")
    print(f"sensitivity_w_adapt_b4.py — complete  ({time.time() - t_start:.1f}s)")
    print(f"{'='*72}")
