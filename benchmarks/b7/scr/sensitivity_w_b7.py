# =============================================================================
# sensitivity_w_b7.py  —  Hybrid Belief Update T-Sensitivity (Benchmark 6)
...
# Date     : 2026-04-25
# Usage    : python scr/sensitivity_w_b7.py
# Outputs  : output/sensitivity_w_results_b7.csv
#            output/sensitivity_w_report_b7.md
# =============================================================================
"""
T-Sensitivity Analysis for Benchmark 7 (Coastal Saltwater Intrusion):
...
Key B5 differences from B4:
  ...
  Conditions: pre-climate (stationary) / sea-level rise shift at t=15
  U_T ≈ 0.7–0.9 (GRACE noise + sparse monitoring — highest in suite)
  SIGMA_OBS=45 (wider than B4=35; GRACE footprint aggregation)

Seeding: M_IDX_BASE=100 to avoid collision with run_benchmark_b7 (m_idx 0-15).
"""
...
from run_benchmark_b7 import _run_episode, _update_belief
...
def main():
    t_start = time.time()
    print("=" * 72)
    print("sensitivity_w_b7.py — T-Sensitivity Analysis (Benchmark 6 Coastal Saltwater Intrusion)")
    ...
    print(f"\n{'='*72}")
    print(f"B7 sensitivity run complete in {elapsed:.1f}s  ({elapsed/60:.1f} min)")

    csv_path = os.path.join(OUTPUT_DIR, "sensitivity_w_results_b7.csv")
    ...
    md_path = os.path.join(OUTPUT_DIR, "sensitivity_w_report_b7.md")
    lines_md = [
